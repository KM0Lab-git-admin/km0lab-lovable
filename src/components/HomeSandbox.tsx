...
const HomeSandbox = ({ state = "guest" }: HomeSandboxProps) => {
  ...
  const modulesWithHandlers: HomeModule[] = useMemo(
    () => seeds.map((m) => ({
      id: m.id,
      active: m.id === "premis" ? state === "registered" || state === "reward-welcome" : m.active,
      disabledReason: m.id === "premis" && !(state === "registered" || state === "reward-welcome") ? "requires_registration" : undefined,
